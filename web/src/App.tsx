...
import { activityStatusTone, conversationBlocks, formatActivityOutcome, formatActivitySummary, type ActivityDetail } from "./activity-detail";
...
const ConversationItemView = memo(function ConversationItemView({ item, highlighted = false, onOpenWorkspaceFile }: { item: NonNullable<SessionSummary["messages"]>[number]; highlighted?: boolean; onOpenWorkspaceFile?: (target: WorkspaceFileTarget) => void }) {
  ...
  if (item.kind === "command" || item.kind === "tool") {
    const tone = activityStatusTone(item);
    return <article id={`message-${item.id}`} className={`tool-card activity-${tone} ${highlighted ? "search-highlight" : ""}`}><span>{item.kind === "command" ? "›_" : "◇"}</span><div><strong>{item.kind === "command" ? "Command" : "Tool"}</strong><p>{item.description || "Working"}</p></div><small>{formatActivityOutcome(item)}</small></article>;
  }
  return (
...
const CollapsedActivityGroup = memo(function CollapsedActivityGroup({ items, onOpenWorkspaceFile }: { items: NonNullable<SessionSummary["messages"]>; onOpenWorkspaceFile?: (target: WorkspaceFileTarget) => void }) {
  return <details className="collapsed-activity"><summary><span aria-hidden="true">◇</span><span><strong>Completed activity</strong><small>{formatActivitySummary(items)}</small></span><i>Details</i></summary><div>{items.map((item) => <ConversationItemView key={item.id} item={item} onOpenWorkspaceFile={onOpenWorkspaceFile} />)}</div></details>;
});

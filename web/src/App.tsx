...
function SettingsView({
  ...
  return <main className="settings-page">
    ...
    <section className="settings-card notification-preferences">
      ...
      <div className="notification-toggle-grid">{eventToggles.map(([key, label]) => <label className="check-row" key={key}><input type="checkbox" checked={notificationPreferences[key] as boolean} onChange={(event) => update(key, event.target.checked)} /><span>{label}</span></label>)}</div>
      ...
      <label className="check-row"><input type="checkbox" checked={notificationPreferences.quietHoursEnabled} onChange={(event) => update("quietHoursEnabled", event.target.checked)} /><span>Quiet hours</span></label>
